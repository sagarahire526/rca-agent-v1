...
embed with OpenAI, and persist everything to Postgres (pwc_agent_utility_schema).

Ported from the simulation agent into the RCA agent repo so this project can build
the shared embedding index from its own `.cypher` scenario nodes. It writes to the
SAME `pwc_agent_utility_schema.nodes` table the RCA app reads from
(services/schema_embedding_service.py).

`scn_agent_type` isolation
--------------------------
Every scenario `:BKGNode` carries a `scn_agent_type` property. It is a value-agnostic
TEXT column here — this repo authors scenario nodes tagged 'rca' or 'recommendation',
the simulation repo tags its own 'simulation'. All three can coexist in the shared
`nodes` table; `search_scenarios(agent_type=...)` slices by the requesting agent so
each agent only matches its own scenarios. `compose_text()` embeds ONLY the scenario's
canonical question so verbatim/lightly-reworded queries match cleanly.

Env: reads NEO4J_* / PG_* / OPENAI_API_KEY from the environment (loaded from ../.env),
falling back to the same defaults the RCA app's config.settings uses — so it works
whether or not NEO4J_* are set in .env.
...
# NEO4J_* are not always in .env — mirror the RCA app's config.settings defaults so
# this standalone script connects to the same graph the app does.
NEO4J_URI = os.environ.get("NEO4J_URI", "neo4j://127.0.0.1:7687")
NEO4J_USER = os.environ.get("NEO4J_USER", "neo4j")
NEO4J_PASSWORD = os.environ.get("NEO4J_PASSWORD", "password")
NEO4J_DATABASE = os.environ.get("NEO4J_DATABASE", "nokia-v-one")

# PG_* defaults mirror services/schema_embedding_service._pg_emb_conn.
PG_HOST = os.environ.get("PG_HOST", "localhost")
PG_PORT = os.environ.get("PG_PORT", "5433")
PG_DATABASE = os.environ.get("PG_DATABASE", "nokia_syn_v1")
