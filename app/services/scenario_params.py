...
resolution. (Ported from the simulation agent, adapted to the RCA agent's LLMProvider.)

The LLM is used ONLY to read scope out of the free-text question into the fixed schema
the scenario node declares (see prompts/scenario_param_prompt.py). It does NOT plan,
choose tables, or write SQL. Everything after extraction is deterministic:

  1. Validate the extracted values against the field allowlists (unknown → default).
...
def _param_llm():
    """Return the shared LLM used for constrained scenario-param extraction.

    RCA's LLMProvider is instance-based (no static "heavy" tier), so we mirror the
    planner's model choice — a strong reasoner is fine for a narrow extraction task.
    """
    from services.llm_provider import LLMProvider
    return LLMProvider(model="gpt-5-mini", reasoning_effort="medium").get_llm()
...
def resolve_build_plan_project_type(project_type_raw: str | None) -> str:
    """Map the user's project-type selection to the value a construction-plan forecast's
    ``build_plan`` expects for ``project_type`` — 'AHLOB' or 'NTM'. Anything not AHLOB
    defaults to NTM."""
...
def extract_scenario_params(
    ...
    """Legacy fixed-4-key extractor (duration/region/group_by). Kept as a fallback for
    scenario nodes that predate ``scn_param_schema.fields``. Never raises — falls back
    to defaults. ``project_type`` is resolved to a single ``smp_name`` deterministically.
    """
