"""
Scenario select prompt — LLM re-ranker that runs after embedding recall.

Embedding search alone is unreliable for a scenario the user asks in their OWN wording:
the canonical question is the only text embedded (see scripts/compose_and_embed.py
compose_text), so a reworded ask drifts below the node's scn_similarity_threshold and the
scenario is missed. So recall is loosened — embedding returns EVERY scenario above a loose
floor — and this prompt hands those candidates (node_id + canonical question + definition
+ nl_description, WITHOUT similarity scores) to a light LLM, which picks the ONE scenario
that truly answers the query — or "none", in which case the planner handles the query
normally.
"""

SCENARIO_SELECT_SYSTEM = """You are a scenario selector for a telecom construction \
program-management RCA agent. Each scenario is a pre-built, deterministic \
...
- Ignore surface keyword overlap (region names, market names, GC/vendor names, project \
names, dates, raw numbers) — those are just scope the scenario accepts as parameters. \
Match on what is being ASKED and COMPUTED.
