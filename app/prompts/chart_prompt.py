...
from the traversal data gathered for the user query. Runs in parallel with
the main response agent and the algorithm narrative LLM, so adding it to
the pipeline costs no wall-clock latency. Output is consumed by the
frontend Highcharts renderer.
"""
