"""
Orchestrator Agent — Routing node for RCA.
...
downstream pipeline to activate:
  ...
  - "rca"        → Root cause investigation; routes to schema discovery → planner → response.
...
_VALID_ROUTES = {"greeting", "traversal", "rca"}


def _parse_orchestrator_response(content: str) -> dict:
    """
    Parse the LLM's JSON routing decision.
    Falls back to "rca" on any parse error (safest default).
    """
    try:
        clean = content.strip()
        if clean.startswith("```"):
        ...
        logger.warning("Orchestrator LLM returned non-JSON; defaulting to rca route.")
        return {
            "routing_decision": "rca",
            "reasoning": "Could not parse routing decision; defaulting to rca.",
            "direct_response": None,
        }


def orchestrator_node(state: RCAState) -> dict[str, Any]:
    """
